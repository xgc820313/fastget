 ...
 *
 *        Version:  1.1
 *        Created:  Friday, Jun 18, 2009 10:59:29 HKT
 *
 *         Author:  dragon, chinagnu@gmail.com
 *        Company:  Fong's National Engineering Co. Ltd
 *        Remarks:  bug fixed , add ftp protocal in download operation
 *
 *
 *        Version:  1.0
...
Task::Task(attr_t m_attr)
{
	...
	if(t_attr.url != NULL && strstr(t_attr.url,"://") != NULL)
	{
		...
		{
			if(strlen(t_attr.user) == 0 || strlen(t_attr.passwd) == 0)
			{
				strncpy(t_attr.user,"anonymous",strlen("anonymous"));
				t_attr.user[strlen("anonymous")]=0;
				strncpy(t_attr.passwd,"i love dragon",strlen("i love dragon"));
				t_attr.user[strlen("i love dragon")]=0;
			}
			...
			if(p_http != NULL)
			{
				if(p_http->get_resource())
				{
					...
					if(f == NULL)
					{
						...
						{
							int len = file_size / (MAX_STRING * MAX_STRING);
							if(len > t_attr.thread_no)
...
//                         }
		...
		case FTP:
			if(p_ftp != NULL)
			{
				while(!p_ftp->get_resource());
				file_size = p_ftp->t_attr->size;
				if(file_size > 0)
				{
					string task_cfg;
					task_cfg.append(getenv("HOME"));
					...
					{  
						...
						if((down_fd[i]=g_thread_create((GThreadFunc)setup_ftp_thread,(void*)&thread_attr[i],true,&thread_error[i]))==NULL)
						{
							g_error_free(thread_error[i]);
						}
					}
					...
					t_attr.start_time = p_ftp->gettime();
				...
				if(strstr(m_http->headers,"HTTP/1.1 206") != NULL || strstr(m_http->headers,"HTTP/1.1 200") != NULL )
				{
					...
					if(body != NULL && m_http->t_attr->size > 0)
					{
						...
						while(m_down->total < m_down->limited)
						{
							...
							int len=fwrite(m_http->headers,sizeof(char),i,fp);  
							if(len > 0)
							{
								fflush(fp);  
								m_http->nbytes += len; 
...
start_down:
	...
	if(m_down->total < m_down->limited)
	{
		Ftp *m_ftp = new Ftp(&m_down->m_attr);
		m_ftp->start_point = m_down->start_address + m_down->total;
		m_ftp->end_point = m_down->end_address;
		m_ftp->nbytes = m_down->total;

		m_ftp->parse_url(m_ftp->t_attr->url);
		m_ftp->create_conn(m_ftp->t_attr->hostname,m_ftp->t_attr->port);
		if(m_ftp->avaliable)
		{
			fseek(fp,m_ftp->start_point,SEEK_SET);

			while(!m_ftp->get_resource());
			if(m_ftp->t_attr->size>0)
			{
				int i=0;
				Conn m_conn;
				m_conn.create_conn(m_ftp->dip,m_ftp->dport);

				if(!m_ftp->split_target(m_ftp->start_point))
				{
					m_ftp->logout();
					m_ftp->dis_conn();
					m_conn.dis_conn();
				...
				while(m_down->total < m_down->limited)
				{
					memset(m_ftp->headers, 0, sizeof(m_ftp->headers));
					i = recv(m_conn.sockfd,m_ftp->headers,MAX_STRING,0);
					...
					int len=fwrite(m_ftp->headers,sizeof(char),i,fp);  
					if(len > 0)
					{
						fflush(fp); 
						m_ftp->nbytes += len; 
						m_down->total = m_ftp->nbytes; 
					}
